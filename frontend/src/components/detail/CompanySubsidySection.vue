<template>
  <n-card class="section-card">
    <template #header>
      <div class="sec-hd">
        <n-icon :component="CashOutline" size="16" color="#4f46e5" />
        <span>補助金情報</span>
...
import { getSubsidies } from '@/api/companies'
...
const columns = [
  { title: '補助金名', key: 'title', ellipsis: { tooltip: true }, minWidth: 200 },
  {
    title: '交付金額',
    key: 'amount',
    width: 130,
...
onMounted(async () => {
  try {
    const data = await getSubsidies(props.corporateNumber)
